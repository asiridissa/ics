...
&Scoped-define INTERNAL-TABLES userCat

/* Definitions for BROWSE arbrw                                         */
&Scoped-define FIELDS-IN-QUERY-arbrw userCat.ID userCat.userCatCode ~
userCat.descrip 
&Scoped-define ENABLED-FIELDS-IN-QUERY-arbrw 
&Scoped-define QUERY-STRING-arbrw FOR EACH userCat NO-LOCK ~
    BY userCat.ID INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-arbrw OPEN QUERY arbrw FOR EACH userCat NO-LOCK ~
    BY userCat.ID INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-arbrw userCat
&Scoped-define FIRST-TABLE-IN-QUERY-arbrw userCat
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS arbrw C-Win _STRUCTURED
  QUERY arbrw NO-LOCK DISPLAY
      userCat.ID FORMAT ">,>>>,>>9":U
      userCat.userCatCode COLUMN-LABEL "       Code" FORMAT "x(3)":U
      userCat.descrip COLUMN-LABEL "                                                            Description" FORMAT "x(50)":U
            WIDTH 56.14
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 13.12 COL 29.86 WIDGET-ID 58
          FGCOLOR 12 
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 14.19 COL 74.86 WIDGET-ID 60
          FGCOLOR 12 
...
/* Query rebuild information for BROWSE arbrw
     _TblList          = "ics.userCat"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _OrdList          = "ics.userCat.ID|yes"
     _FldNameList[1]   = ics.userCat.ID
     _FldNameList[2]   > ics.userCat.userCatCode
"userCat.userCatCode" "       Code" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > ics.userCat.descrip
"userCat.descrip" "                                                            Description" ? "character" ? ? ? ? ? ? no ? no no "56.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE arbrw */
&ANALYZE-RESUME
...
DO:
  ...
  FIND FIRST paramtrs WHERE NAME = "lastUserCatCode".
...
DO:
  ...
  OPEN QUERY arbrw FOR EACH userCat BY ID.
...
DO:
    ...
    DO:
        ...
        IF yn THEN
        DO:
            FIND FIRST userCat WHERE userCatCode = filCode.
            IF AVAILABLE userCat THEN
                DELETE userCat.
...
/********************************************save************************************************/
    ...
    DO:
        FIND FIRST userCat WHERE userCatCode = filCode OR descrip  = filDescr EXCLUSIVE-LOCK NO-ERROR.
            IF AVAILABLE userCat THEN
            DO:
                MESSAGE "Area Code or Description already exists.1" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
                RETURN.
            END.
            ELSE IF NOT AVAILABLE userCat THEN
            DO:
                CREATE userCat.
                ID       = filID.
                userCatCode = filCode.
    ...
    DO:
        FOR EACH userCat WHERE userCatCode = filCode OR userCat.descrip  = filDescr.
        ...
        DO:
            FIND FIRST userCat WHERE ID = filID EXCLUSIVE-LOCK NO-ERROR.
                ASSIGN
                    ID       = filID
                    userCatCode = filCode
                    userCat.descrip  = filDescr.
        END.
...
ON ANY-KEY OF filCode IN FRAME DEFAULT-FRAME /* User Category Code */
DO:
  ASSIGN filCode.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL filCode C-Win
ON LEAVE OF filCode IN FRAME DEFAULT-FRAME /* User Category Code */
DO:
  ASSIGN {&SELF-NAME}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL filCode C-Win
ON RETURN OF filCode IN FRAME DEFAULT-FRAME /* User Category Code */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL filCode C-Win
ON VALUE-CHANGED OF filCode IN FRAME DEFAULT-FRAME /* User Category Code */
DO:
  ASSIGN filCode.
  filCode = CAPS(filCode).
  DISPLAY filCode WITH FRAME {&FRAME-NAME}.

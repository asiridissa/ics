...
&Scoped-define FIELDS-IN-QUERY-arbrw area.ID area.areaCode area.descrip 
&Scoped-define ENABLED-FIELDS-IN-QUERY-arbrw 
&Scoped-define QUERY-STRING-arbrw FOR EACH area NO-LOCK ~
    BY area.ID INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-arbrw OPEN QUERY arbrw FOR EACH area NO-LOCK ~
    BY area.ID INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-arbrw area
&Scoped-define FIRST-TABLE-IN-QUERY-arbrw area
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS arbrw C-Win _STRUCTURED
  QUERY arbrw NO-LOCK DISPLAY
      area.ID FORMAT ">,>>>,>>9":U WIDTH 5.43
      area.areaCode COLUMN-LABEL "   Code" FORMAT "x(8)":U
      area.descrip COLUMN-LABEL "                                                          Discription" FORMAT "x(100)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 13.12 COL 26.29 WIDGET-ID 56
          FGCOLOR 12 
     "*" VIEW-AS TEXT
...
/* Query rebuild information for BROWSE arbrw
     ...
     _OrdList          = "ics.area.ID|yes"
     _FldNameList[1]   > ics.area.ID
"area.ID" ? ? "integer" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > ics.area.areaCode
"area.areaCode" "   Code" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > ics.area.descrip
"area.descrip" "                                                          Discription" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE arbrw */
&ANALYZE-RESUME
...
DO:
  ...
  FIND FIRST paramtrs WHERE NAME = "lastAreaID".
...
DO:
  ...
  OPEN QUERY arbrw FOR EACH area BY ID.
...
DO:
    ...
    DO:
        ...
        IF yn THEN
        DO:
            FIND FIRST area WHERE areaCode = filCode.
            IF AVAILABLE area THEN
                DELETE area.
...
/********************************************save************************************************/
    ...
    DO:
        FIND FIRST area WHERE areaCode = filCode OR descrip  = filDescr EXCLUSIVE-LOCK NO-ERROR.
            IF AVAILABLE area THEN
            DO:
                MESSAGE "Area Code or Description already exists.1" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
                RETURN.
            END.
            ELSE IF NOT AVAILABLE area THEN
            DO:
                CREATE area.
                ID       = filID.
                areaCode = filCode.
    ...
    DO:
        FOR EACH area WHERE areaCode = filCode OR area.descrip  = filDescr.
        ...
        DO:
            FIND FIRST area WHERE ID = filID EXCLUSIVE-LOCK NO-ERROR.
                ASSIGN
                    ID       = filID
                    areaCode = filCode
                    area.descrip  = filDescr.
        END.

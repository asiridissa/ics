...
&Scoped-define INTERNAL-TABLES CusCat

/* Definitions for BROWSE arbrw                                         */
&Scoped-define FIELDS-IN-QUERY-arbrw CusCat.ID CusCat.catName ~
CusCat.Descrip 
&Scoped-define ENABLED-FIELDS-IN-QUERY-arbrw 
&Scoped-define QUERY-STRING-arbrw FOR EACH CusCat NO-LOCK ~
    BY CusCat.catName INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-arbrw OPEN QUERY arbrw FOR EACH CusCat NO-LOCK ~
    BY CusCat.catName INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-arbrw CusCat
&Scoped-define FIRST-TABLE-IN-QUERY-arbrw CusCat
...
DEFINE VARIABLE filCode AS CHARACTER FORMAT "X(20)":U 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS arbrw C-Win _STRUCTURED
  QUERY arbrw NO-LOCK DISPLAY
      CusCat.ID FORMAT ">>>>>>>9":U
      CusCat.catName FORMAT "x(20)":U
      CusCat.Descrip FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     filID AT ROW 12.04 COL 21.57 COLON-ALIGNED WIDGET-ID 16
     filCode AT ROW 13.12 COL 21.57 COLON-ALIGNED WIDGET-ID 2
     filDescr AT ROW 14.19 COL 21.57 COLON-ALIGNED WIDGET-ID 4
     ...
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 13.12 COL 44.57 WIDGET-ID 58
          FGCOLOR 12 
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 14.19 COL 74.43 WIDGET-ID 60
          FGCOLOR 12 
...
/* Query rebuild information for BROWSE arbrw
     ...
     _OrdList          = "ics.CusCat.catName|yes"
     _FldNameList[1]   = ics.CusCat.ID
     _FldNameList[2]   = ics.CusCat.catName
     _FldNameList[3]   = ics.CusCat.Descrip
     _Query            is OPENED
*/  /* BROWSE arbrw */
&ANALYZE-RESUME
...
ON END-ERROR OF C-Win /* ICS - User Categories */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* ICS - User Categories */
DO:
...
DO:
    IF AVAILABLE CusCat THEN
        ASSIGN
        filID    = CusCat.ID. 
        filCode  = CusCat.catName.
        filDescr = CusCat.Descrip. 
...
DO:
  ...
  FIND FIRST paramtrs WHERE NAME = "lastCusCatID".
...
DO:
  ...
  OPEN QUERY arbrw FOR EACH CusCat BY catName.
...
DO:
    IF filCode = "Other" THEN
    DO:
      MESSAGE "You cannot change this Category." VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN.
    END.
    IF filCode = "" THEN
    ...
    DO:
        ...
        IF yn THEN
        DO:
            FIND FIRST CusCat WHERE CusCat.catName = filCode.
            IF AVAILABLE CusCat THEN
                DELETE CusCat.
...
DO:
  IF filID <= 2 THEN
  DO:
      MESSAGE "You cannot change this Category." VIEW-AS ALERT-BOX WARNING BUTTONS OK.
      RETURN.
  END.
...
/********************************************save************************************************/
    ...
    DO:
        FIND FIRST CusCat WHERE CusCat.catName = filCode OR CusCat.Descrip = filDescr EXCLUSIVE-LOCK NO-ERROR.
            IF AVAILABLE CusCat THEN
            DO:
                MESSAGE "Customer Category or Description already exists." VIEW-AS ALERT-BOX ERROR BUTTONS OK.
                RETURN.
            END.
            ELSE IF NOT AVAILABLE CusCat THEN
            DO:
                CREATE CusCat.
                ID       = filID.
                catName = filCode.
    ...
    DO:
        FOR EACH CusCat WHERE CusCat.catName = filCode OR CusCat.Descrip  = filDescr.
            ModifyCnt = ModifyCnt + 1.
        END.
        IF ModifyCnt > 1 THEN
        DO:
            MESSAGE "Area Code or Description already exists." VIEW-AS ALERT-BOX ERROR BUTTONS OK.
            ModifyCnt = 0.
        ...
        DO:
            FIND FIRST CusCat WHERE ID = filID EXCLUSIVE-LOCK NO-ERROR.
                ASSIGN
                    ID       = filID
                    CusCat.catName = filCode
                    CusCat.Descrip  = filDescr.
        END.
...
ON LEAVE OF filCode IN FRAME DEFAULT-FRAME /* Customer Category Code */
DO:
  ASSIGN {&SELF-NAME}.
  filCode = CAPS(substring(filCode,1,1)) + substring(filCode,2).

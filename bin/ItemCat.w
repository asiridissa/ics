...
&Scoped-define INTERNAL-TABLES itemCat

/* Definitions for BROWSE arbrw                                         */
&Scoped-define FIELDS-IN-QUERY-arbrw itemCat.ID itemCat.itemCatCode ~
itemCat.descrip 
&Scoped-define ENABLED-FIELDS-IN-QUERY-arbrw 
&Scoped-define QUERY-STRING-arbrw FOR EACH itemCat NO-LOCK ~
    BY itemCat.ID INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-arbrw OPEN QUERY arbrw FOR EACH itemCat NO-LOCK ~
    BY itemCat.ID INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-arbrw itemCat
&Scoped-define FIRST-TABLE-IN-QUERY-arbrw itemCat
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS arbrw C-Win _STRUCTURED
  QUERY arbrw NO-LOCK DISPLAY
      itemCat.ID FORMAT ">,>>>,>>9":U
      itemCat.itemCatCode COLUMN-LABEL "    Code" FORMAT "x(3)":U
      itemCat.descrip COLUMN-LABEL "                                                         Description" FORMAT "x(50)":U
            WIDTH 56.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     filID AT ROW 12.04 COL 21.43 COLON-ALIGNED WIDGET-ID 16
     filCode AT ROW 13.12 COL 21.43 COLON-ALIGNED WIDGET-ID 2
     filDescr AT ROW 14.19 COL 21.43 COLON-ALIGNED WIDGET-ID 4
     ...
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 13.12 COL 29.72 WIDGET-ID 60
          FGCOLOR 12 
     "*" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 14.19 COL 74.72 WIDGET-ID 62
          FGCOLOR 12 
...
/* Query rebuild information for BROWSE arbrw
     _TblList          = "ics.itemCat"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _OrdList          = "ics.itemCat.ID|yes"
     _FldNameList[1]   = ics.itemCat.ID
     _FldNameList[2]   > ics.itemCat.itemCatCode
"itemCat.itemCatCode" "    Code" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > ics.itemCat.descrip
"itemCat.descrip" "                                                         Description" ? "character" ? ? ? ? ? ? no ? no no "56.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE arbrw */
&ANALYZE-RESUME
...
ON END-ERROR OF C-Win /* LMS - Item Categories */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* LMS - Item Categories */
DO:
...
DO:
    IF AVAILABLE itemCat THEN
        ASSIGN
        filID    = ID
        filCode  = itemCatCode
        filDescr = descrip.
    DISPLAY filCode filDescr filID WITH FRAME DEFAULT-FRAME.
END.
...
DO:
  ...
  FIND FIRST paramtrs WHERE NAME = "lastItemCatCode".
...
DO:
  ...
  OPEN QUERY arbrw FOR EACH itemCat BY ID.
...
DO:
    ...
    DO:
        ...
        IF yn THEN
        DO:
            FIND FIRST itemCat WHERE itemCatCode = filCode.
            IF AVAILABLE itemCat THEN
                DELETE itemCat.
...
/********************************************save************************************************/
    ...
    DO:
        FIND FIRST itemCat WHERE itemCatCode = filCode OR descrip  = filDescr EXCLUSIVE-LOCK NO-ERROR.
            IF AVAILABLE itemCat THEN
            DO:
                MESSAGE "Area Code or Description already exists.1" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
                RETURN.
            END.
            ELSE IF NOT AVAILABLE itemCat THEN
            DO:
                CREATE itemCat.
                ID       = filID.
                itemCatCode = filCode.
    ...
    DO:
        FOR EACH itemCat WHERE itemCatCode = filCode OR itemCat.descrip  = filDescr.
        ...
        DO:
            FIND FIRST itemCat WHERE ID = filID EXCLUSIVE-LOCK NO-ERROR.
                ASSIGN
                    ID       = filID
                    itemCatCode = filCode
                    itemCat.descrip  = filDescr.
        END.
...
ON ANY-KEY OF filCode IN FRAME DEFAULT-FRAME /* Item Category Code */
DO:
  ASSIGN filCode.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL filCode C-Win
ON LEAVE OF filCode IN FRAME DEFAULT-FRAME /* Item Category Code */
DO:
  ASSIGN {&SELF-NAME}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL filCode C-Win
ON RETURN OF filCode IN FRAME DEFAULT-FRAME /* Item Category Code */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL filCode C-Win
ON VALUE-CHANGED OF filCode IN FRAME DEFAULT-FRAME /* Item Category Code */
DO:
  ASSIGN filCode.
  filCode = CAPS(filCode).
  DISPLAY filCode WITH FRAME {&FRAME-NAME}.
...
ON ANY-KEY OF filDescr IN FRAME DEFAULT-FRAME /* Description */
DO:
  ASSIGN filDescr.
